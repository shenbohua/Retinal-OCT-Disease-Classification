...
def command_draw_learning(args: argparse.Namespace) -> None:
    """Plot learning curves from all discovered DL training histories."""
    paths = make_paths()
    ensure_dirs([paths.figures_root, paths.logs_root])
    logger = setup_logger(paths.logs_root / "draw_learning.log")

    run_info = discover_dl_runs()
    if not run_info:
        logger.warning("No DL run data found. Run `python main.py train-dl` first.")
        print("No DL run data found under outputs/runs/deeplearning/.")
        return

    model_filter = args.models
    if model_filter:
        run_info = {k: v for k, v in run_info.items() if k in model_filter}
        if not run_info:
            logger.warning("No matching models found for filter: %s", model_filter)
            print(f"No runs matched models: {model_filter}")
            return

    print(f"Found {len(run_info)} DL runs: {', '.join(run_info.keys())}")
    histories = load_histories_dict({name: info["history"] for name, info in run_info.items()})
    figures = plot_all_learning_curves(histories, save_dir=paths.figures_root)
    logger.info("Generated %d learning-curve figures.", len(figures))


def command_draw_roc(args: argparse.Namespace) -> None:
    """Plot ROC curves from all discovered DL prediction files."""
    paths = make_paths()
    ensure_dirs([paths.figures_root, paths.logs_root])
    logger = setup_logger(paths.logs_root / "draw_roc.log")
    ...
    print(f"Found {len(run_info)} DL runs: {', '.join(run_info.keys())}")
    predictions = load_predictions_dict({name: info["predictions"] for name, info in run_info.items()})
    figures = plot_all_roc_curves(predictions, save_dir=paths.figures_root)
    logger.info("Generated %d ROC figures.", len(figures))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    draw_l = subparsers.add_parser("draw-learning", help="Plot learning curves from DL training histories")
    draw_l.add_argument(
        "--models", nargs="*", default=None,
        choices=["resnet18", "resnet34", "resnet50", "vgg16", "mobilenet_v2"],
        help="Specific models to plot (default: all discovered)",
    )
    draw_l.set_defaults(func=command_draw_learning)

    draw_r = subparsers.add_parser("draw-roc", help="Plot ROC curves from DL prediction probabilities")
    draw_r.add_argument(
        "--models", nargs="*", default=None,
        choices=["resnet18", "resnet34", "resnet50", "vgg16", "mobilenet_v2"],
        help="Specific models to plot (default: all discovered)",
    )
    draw_r.set_defaults(func=command_draw_roc)
