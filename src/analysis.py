...
def build_traditional_summary(tables_root: Path) -> pd.DataFrame:
    ...
    frames = []
    direct_result_files = sorted(tables_root.glob("result_*.csv"))
    run_result_files = sorted((tables_root.parent / "runs").glob("**/tables/result.csv"))
    seen = set()
    for path in direct_result_files + run_result_files:
        if path in seen:
            continue
        seen.add(path)
        df = pd.read_csv(path)
        if not df.empty:
            frames.append(df.assign(result_file=str(path.relative_to(tables_root.parent)), run_type="validation"))
    ...
    dedupe_cols = [c for c in ["feature", "classifier", "eval_split", "run_dir"] if c in summary.columns]
    if dedupe_cols:
        summary = summary.drop_duplicates(subset=dedupe_cols, keep="last")
